...
class Parser {               // essencial
   ...
   ************************************************/
    ...
    void MONTAwhile(String exp){
        p.push(new CmdWhile(exp));
    };
    ...
    void ANINHA(){
        ...
        if(p.vazia())
            ...
            Programa topo = p.getTopo();
            topo.push(cmd);
        }
    }
   
    void MONTAdecl(Tipo tipo, String exp){
        Programa cmd1 = new CmdDecl(tipo, exp);
        
        if(p.vazia()){
            prog.push(cmd1);
        }else{
            console("MONTAdecl: deu grozopi");
        }
   };
   
    void MONTAatrib(String var, String exp){
        Programa cmd1 = new CmdAtrib(var, exp);
    ...
    void MONTAinput(Tipo tip, String var){
        Programa cmd1 = new CmdInput(tip, var);
    ...
    void MONTAprint(String exp){
        Programa cmd1 = new CmdPrint(exp);;
