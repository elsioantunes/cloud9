...
class Parser {
    ...
    public static void addErros(int code, int tipo, String msg, int lin, int col)
    {
        erros.add(new Erro(code, tipo, msg, lin,col));
    }
   /***********************************************
   ************************************************/
   ...
   ************************************************/
    void noExpected(){
        if(is(TokenTypes.TOKEN_INVALIDO)) {ignore("token inválido");}
            ...
            nextToken(); 
            erros.add(new Erro(666,1,"sei lá o que pode ser",LIN,COL));
            //erro(" sei lá o que pode ser "+CODE+" neste contexto.");
        }
    }
    void ignore(String str){
        erros.add(new Erro(10,0,"não esperava "+str+" neste momento",LIN,COL));
   ...
   ************************************************/
    ...
    void match(TokenTypes code){
        ...
        else erros.add(new Erro(80,1," esperava "+code+" mas veio "+CODE,LIN,COL));//erro (" esperava "+code+" mas veio "+CODE);
    }
