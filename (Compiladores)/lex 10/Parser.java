...
class Parser {               // essencial
   ...
   ************************************************/
    ...
    void MONTAdecl(Tipo tipo, String exp){
        cmd1 = new CmdDecl(tipo, exp);
        if(p.vazia()){
            prog.push(cmd1);
        }else{
            cmd2 = p.getTopo();
            ((CmdIf)cmd2).push(cmd1);
        }
   };
   
    void MONTAatrib(String var, String exp){
        cmd1 = new CmdAtrib(var, exp);
        if(p.vazia()){
            prog.push(cmd1);
        }else{
            cmd2 = p.getTopo();
    ...
    void MONTAprint(String exp){
        cmd1 = new CmdPrint(exp);
        if(p.vazia()){
            prog.push(cmd1);
        }else{
            cmd2 = p.getTopo();
   ...
   ************************************************/
    Var Opera(Var a, int op, Var b){
        ...
        if(a.tipo == Tipo.STRING && op != 0) erro("operador inválido");
        ...
        switch(op){
            ...
            //  "!="  | ">="  | "<="  | "<"   | ">" 
            case 4: if(x1!=y1 && x2!=y2) x4 = true; else x4 = false; break;
            ...
            case 8: if(x1==y1 && x2==y2) x4 = true; else x4 = false; break;
